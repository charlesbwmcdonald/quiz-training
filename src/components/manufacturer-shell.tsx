...
export async function ManufacturerHeader({ brand, email }: { brand: ManufacturerBrand; email?: string | null }) {
  ...
  return <header className="border-b border-black/10 bg-white">
    <div className="mx-auto flex max-w-7xl items-center justify-between gap-5 px-5 py-4 lg:px-8">
      <div className="flex min-w-0 items-center gap-3"><Link href={portal} prefetch={false} aria-label={`${brand.name} academy dashboard`} className="flex min-w-0 items-center gap-4">
      ...
      <nav aria-label="Main navigation" className="flex items-center gap-2 text-sm font-bold uppercase tracking-wide sm:gap-4">
        <details className="group relative">
          ...
          <div className="absolute right-0 z-50 mt-2 overflow-hidden border border-black/10 bg-white py-1 shadow-xl">
            <Link href={portal} prefetch={false} className={menuLink}>Quizzes</Link>
            ...
            <div className="mt-1 border-t border-black/10 pt-1"><Link href={`${portal}/my-training`} prefetch={false} className={menuLink} style={{ color: brand.primary_color }}>My Training</Link><Link href="/academies" prefetch={false} className={menuLink} style={{ color: brand.primary_color }}>My Academies</Link></div>
          </div>
        </details>
