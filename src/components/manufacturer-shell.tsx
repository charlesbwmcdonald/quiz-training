      ...
      <nav aria-label="Main navigation" className="flex items-center gap-2 text-sm font-bold uppercase tracking-wide sm:gap-4">
        <details className="group relative">
          ...
          <div className="absolute right-0 z-50 mt-2 overflow-hidden border border-black/10 bg-white py-1 shadow-xl">
            <Link href={portal} className={menuLink}>Training</Link>
            <Link href={`${portal}/team-training`} className={menuLink}>Team Training</Link>
            <Link href={`${portal}/products`} className={menuLink}>Products</Link>
            {brand.can_manage_training && <Link href={`${portal}/products/import`} className={menuLink}>Import Products</Link>}
            {brand.can_manage_training && <Link href={`${portal}/courses`} className={menuLink}>Courses</Link>}
          </div>
        </details>
        {(brand.can_manage_training || brand.can_view_reports || brand.can_manage_brand) && <details className="group relative">
          ...
          <div className="absolute right-0 z-50 mt-2 overflow-hidden border border-black/10 bg-white py-1 shadow-xl">
            {brand.can_manage_training && <Link href={`${portal}/retailers`} className={menuLink}>Retailers</Link>}
            {brand.can_manage_training && <Link href={`${portal}/assignments`} className={menuLink}>Assignments</Link>}
            {brand.can_view_reports && <Link href={`${portal}/reports`} className={menuLink}>Reports</Link>}
            {brand.can_manage_brand && <Link href={`${portal}/users`} className={menuLink}>Users</Link>}
            {brand.can_manage_brand && <Link href={`${portal}/settings/branding`} className={menuLink} style={{ color: brand.primary_color }}>Brand</Link>}
          </div>
        </details>}
