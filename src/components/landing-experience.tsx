...
export function ProductCarousel({ products, manufacturerSlug, color, autoplay }: { products: FeaturedProduct[]; manufacturerSlug: string; color: string; autoplay: boolean }) {
  const [index, setIndex] = useState(0);
  const [motionAllowed, setMotionAllowed] = useState(false);
  useEffect(() => setMotionAllowed(!window.matchMedia("(prefers-reduced-motion: reduce)").matches), []);
  useEffect(() => {
