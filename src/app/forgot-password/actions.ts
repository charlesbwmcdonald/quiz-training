...
export async function requestPasswordReset(formData:FormData){
  ...
  const next=`/reset-password${brand?`?brand=${encodeURIComponent(brand)}`:""}`;
  const supabase=await createSupabaseServerClient();
  await supabase.auth.resetPasswordForEmail(email,{redirectTo:`${site}/auth/callback?next=${encodeURIComponent(next)}`});
