...
export type EditableCourse = { id: string; title: string; description: string; status: "draft" | "review" | "published"; hasActivity: boolean; blocks: EditableCourseBlock[] };
...
export default function CourseBuilder({ quizzes, products, primary, error, initialCourse }: { quizzes: Quiz[]; products: Product[]; primary: string; error?: string; initialCourse?: EditableCourse }) {
  ...
  return <form action={editing ? updateCourse : createCourse} className="mt-8 grid gap-6">
    ...
    <section className="rounded-lg border border-black/10 bg-white p-6 shadow-sm"><h2 className="text-xl font-extrabold uppercase">Course details</h2><label className="mt-5 grid gap-2 font-bold">Course title<input name="title" required defaultValue={initialCourse?.title} className="min-h-12 border px-4 font-normal" /></label><label className="mt-5 grid gap-2 font-bold">Description<textarea name="description" rows={3} defaultValue={initialCourse?.description} className="border p-4 font-normal" /></label></section>
    {blocks.map((block, index) => <section key={block.id} className="rounded-lg border border-black/10 bg-white p-6 shadow-sm">
    ...
    <div className="flex flex-wrap justify-end gap-3"><button name="intent" value="draft" className={academyButton.secondary}>{initialCourse?.status === "published" ? "Move to draft" : "Save draft"}</button><button name="intent" value="review" className={academyButton.secondary}>Send for review</button><button name="intent" value="published" className={academyButton.primary} style={{ backgroundColor: primary }}>{initialCourse?.status === "published" ? "Save changes" : editing ? "Save & publish" : "Publish course"}</button></div>
  </form>;
}
