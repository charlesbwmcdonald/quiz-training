...
export default async function CoursesPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <div className="min-h-screen bg-[#f4f4f2]">
    ...
    <main className="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-14">
      ...
      <section className="mt-8 overflow-hidden rounded-lg border border-black/10 bg-white shadow-sm"><div className="hidden grid-cols-[minmax(0,1.5fr)_100px_120px_120px_150px_110px] items-center gap-5 border-b border-black/10 bg-black/[.03] px-5 py-4 text-xs font-extrabold uppercase tracking-wide text-black/45 lg:grid"><span>Course</span><span>Status</span><span>Content</span><span>Activity</span><span>Audience</span><span className="justify-self-end">Actions</span></div>{((courses as Course[]) ?? []).map((course) => {
