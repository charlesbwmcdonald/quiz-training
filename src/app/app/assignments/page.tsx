...
export default async function AssignmentsPage({searchParams}:{searchParams:Promise<{error?:string;saved?:string;updated?:string;removed?:string;notified?:string;emailFailed?:string}>}){
  ...
  return <div className="min-h-screen bg-[#f4f4f2] text-black"><ManufacturerHeader brand={brand} email={auth.user.email}/><main className="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-14">
    ...
    {params.notified&&<div role="status" className="mt-3 border-l-4 border-green-600 bg-green-50 p-4 font-semibold text-green-900">Assignment email sent to {params.notified} learner{params.notified==="1"?"":"s"}.{Number(params.emailFailed)>0?` ${params.emailFailed} could not be delivered.`:""}</div>}
