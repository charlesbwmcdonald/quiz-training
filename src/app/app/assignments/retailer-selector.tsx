...
export default function RetailerSelector({retailers}: {retailers: AssignmentRetailer[]}) {
  ...
  const selectAll = (checked: boolean) => {
    setSelectedIds(current => {
      ...
      visibleRetailers.forEach(retailer => checked ? next.add(retailer.company_id) : next.delete(retailer.company_id));
      return next;
    });
  };
  const selectedVisible = visibleRetailers.filter(retailer => selectedIds.has(retailer.company_id)).length;
