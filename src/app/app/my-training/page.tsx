...
export default async function MyTrainingPage({ searchParams }:{ searchParams:Promise<{ filter?:string }> }) {
  ...
  return <div className="min-h-screen bg-[#f4f4f2]"><ManufacturerHeader brand={brand} email={auth.user.email}/><main className="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-14">
    ...
    <section className="mt-5 overflow-hidden overflow-hidden rounded-lg border border-black/10 bg-white shadow-sm"><div className="hidden grid-cols-[minmax(0,1.5fr)_90px_150px_190px_130px_110px_110px] items-center gap-5 border-b border-black/10 bg-black/[.03] px-5 py-4 text-xs font-extrabold uppercase tracking-wide text-black/45 lg:grid"><span>Training</span><span>Type</span><span>Assigned by</span><span>Progress</span><span>Due date</span><span>Status</span><span className="justify-self-end">Action</span></div>
