...
export default function QuizCardActions({ quizId, status, hasAttempts, brandSlug, assignedInternally }: { quizId: string; status: "draft" | "review" | "published" | "archived"; hasAttempts: boolean; brandSlug: string; assignedInternally: boolean }) {
  ...
  return <details className="group relative justify-self-end"><summary className="academy-control-trigger flex min-h-10 cursor-pointer list-none items-center gap-3 px-4 text-xs font-extrabold uppercase marker:content-none [&::-webkit-details-marker]:hidden">Manage <span className="text-[9px] transition group-open:rotate-180">▼</span></summary><div className="absolute right-0 z-40 mt-2 w-56 rounded-lg border border-black/15 bg-white py-2 shadow-2xl">
