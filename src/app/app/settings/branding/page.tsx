...
import { BrandStudioSaveBar } from "@/components/brand-studio-save-bar";
...
const area = `${academyInput} min-h-24 py-3`;
...
export default async function BrandingSettingsPage({ searchParams }: { searchParams: Promise<{error?:string;saved?:string;mode?:string}> }) {
  ...
  return <div className="min-h-screen bg-[#f4f4f2] text-black"><ManufacturerHeader brand={activeBrand} email={auth.user.email}/><main className="mx-auto max-w-[1600px] px-5 py-10 lg:px-8 lg:py-14">
    <AcademyPageHeader eyebrow="Manufacturer settings" title="Brand Studio" description="Control your identity, sign-in experience, landing-page content, featured products, and promotions." accent={brand.primary_color} actions={<Link href={`/m/${brand.slug}`} target="_blank" className={academyButton.secondary}>View landing page ↗</Link>}/>
    ...
    <form action={updateBranding} className="mt-8 grid gap-5"><input type="hidden" name="manufacturerId" value={brand.id}/><input type="hidden" name="slug" value={brand.slug}/>
      <AcademySurface className="p-6 sm:p-7"><StepLabel number="01" accent={brand.primary_color}>Academy identity</StepLabel><h2 className="mt-3 text-2xl font-extrabold uppercase">Brand identity</h2><div className="mt-6 grid gap-5"><label className="grid gap-2 font-bold">Manufacturer name<input name="name" defaultValue={brand.name} required className={input}/></label><label className="grid gap-2 font-bold">Logo <span className="text-xs font-normal text-black/45">PNG, JPG, or WebP</span><input name="logo" type="file" accept="image/png,image/jpeg,image/webp" className="min-h-12 rounded-md border border-black/15 bg-white p-3 font-normal"/></label><div className="grid gap-5 sm:grid-cols-2"><BrandColorField label="Primary color" name="primaryColor" initialValue={brand.primary_color}/><BrandColorField label="Secondary color" name="secondaryColor" initialValue={brand.secondary_color}/></div></div></AcademySurface>

      <AcademySurface className="p-6 sm:p-7"><StepLabel number="02" accent={brand.primary_color}>Learner entry point</StepLabel><h2 className="mt-3 text-2xl font-extrabold uppercase">Branded sign-in experience</h2><p className="mt-2 text-sm text-black/55">Displayed when learners sign in through this manufacturer’s landing page.</p><div className="mt-6 grid gap-5"><label className="grid gap-2 font-bold">Sign-in background image<input name="loginImage" type="file" accept="image/png,image/jpeg,image/webp" className="min-h-12 rounded-md border border-black/15 bg-white p-3 font-normal"/></label><label className="grid gap-2 font-bold">Welcome headline<input name="loginHeadline" maxLength={120} defaultValue={experience.login?.headline ?? "Welcome to your training center"} className={input}/></label><label className="grid gap-2 font-bold">Welcome message<textarea name="loginDescription" maxLength={400} rows={3} defaultValue={experience.login?.description ?? "Sign in to continue your product training."} className={area}/></label></div></AcademySurface>

      <PageBuilder initialSections={builderSections} products={products} primary={brand.primary_color}/>
      <BrandStudioSaveBar primary={brand.primary_color} initiallySaved={Boolean(params.saved)}/>
    </form>
  </main></div>;
}
