...
export default async function NewQuizPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  return <div className="min-h-screen bg-[#f4f4f2] text-black" style={{ "--brand-primary": brand.primary_color } as React.CSSProperties}><ManufacturerHeader brand={brand} email={auth.user?.email} /><main className="px-5 py-10 lg:px-8"><div className="mx-auto max-w-4xl"><Link href="/app" className="text-sm font-bold uppercase tracking-wide text-black/60 hover:underline">← Quiz library</Link><div className="mt-8"><AcademyPageHeader eyebrow={`${brand.name} quiz builder`} title="Create a Quiz" description="Add questions and identify one correct answer for each. Save a draft or publish when it is ready." accent={brand.primary_color}/></div><QuizBuilder error={error} primaryColor={brand.primary_color} productImages={productImages} /></div></main></div>;
}
