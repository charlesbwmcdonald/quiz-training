...
export default function QuizBuilder({ error, primaryColor, initial, quizId }: { error?: string; primaryColor: string; initial?: { title: string; description: string; passingScore: number; status: "draft" | "published"; questions: Question[] }; quizId?: string }) {
    ...
    <form action={quizId ? updateQuiz : createQuiz} className="mt-8 grid gap-6">
      ...
      {questions.map((question, questionIndex) => (
        <section key={question.id} className="border border-black/10 bg-white p-6 shadow-sm sm:p-8">
          ...
          <label className="mt-4 grid gap-2 font-bold">Question image URL <span className="text-xs font-normal text-black/45">Optional · learners can click the image to enlarge it</span><input value={question.imageUrl} onChange={(e) => updateQuestionImage(question.id, e.target.value)} placeholder="https://…" className="min-h-11 border border-black/20 px-4 font-normal outline-none focus:border-[#d90000]" /></label>
