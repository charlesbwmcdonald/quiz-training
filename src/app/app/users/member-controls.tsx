...
export default function MemberControls({ userId, email, scope, companyId, role, currentUserId }: { userId:string; email:string; scope:"manufacturer"|"retailer"; companyId?:string; role:string; currentUserId:string }) {
  ...
  return <details className="group relative justify-self-end">
    <summary className="academy-control-trigger flex min-h-10 cursor-pointer list-none items-center gap-3 px-4 text-xs font-extrabold uppercase marker:content-none [&::-webkit-details-marker]:hidden">Manage <span aria-hidden="true" className="text-[9px] transition group-open:rotate-180">▼</span></summary>
    <div className="absolute right-0 z-40 mt-2 w-72 rounded-lg border border-black/15 bg-white p-4 text-left shadow-2xl">
