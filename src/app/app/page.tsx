...
import { AcademyPageHeader, academyButton } from "@/components/academy-ui";
...
type Quiz = { id: string; title: string; description: string | null; status: "draft" | "review" | "published" | "archived"; passing_score: number; created_at: string; quiz_questions: { count: number }[] };
...
export default async function AppPage({ searchParams }: { searchParams: Promise<{ error?: string }> }) {
  ...
  const [{ data, error }, { data: attemptCounts }, { data: courseData }, { data: internalQuizRows }, { data: governanceRows }, {data:scheduleRows}] = await Promise.all([
    ...
    isManager ? supabase.rpc("manufacturer_content_governance") : Promise.resolve({ data: [] }),
    isManager ? supabase.rpc("manufacturer_content_schedules") : Promise.resolve({data:[]}),
  ]);
  ...
  const governance = new Map(((governanceRows ?? []) as ContentGovernanceRecord[]).filter((item)=>item.content_type==="quiz").map((item)=>[item.content_id,item]));
  const schedules = new Map(((scheduleRows??[]) as ContentSchedule[]).filter(item=>item.content_type==="quiz"&&item.schedule_status==="pending").map(item=>[item.content_id,item]));
  ...
  return <div className="min-h-screen bg-[#f4f4f2] text-black">
    ...
    <main className="mx-auto max-w-7xl px-5 py-10 lg:px-8 lg:py-14">
      ...
      <AcademyPageHeader eyebrow={isManager ? "Learning library" : `${brand?.name ?? "Product"} training`} title={isManager ? "Quiz Library" : "My Training"} description={isManager ? "Create product-training quizzes, prepare questions, and publish them when they are ready for your dealer network." : "Complete assigned product training and keep your knowledge current."} accent={primary} actions={isManager?<><Link href={`/m/${brand?.slug}/app/quizzes/import`} className={academyButton.secondary}>Import CSV</Link><Link href={`/m/${brand?.slug}/app/quizzes/new`} className={academyButton.primary} style={{ backgroundColor: primary }}>Create Quiz</Link></>:undefined}/>
      ...
      {!isManager ? (
        ...
        <section className="mt-8 overflow-hidden rounded-lg border border-black/10 bg-white shadow-sm"><div className="hidden grid-cols-[minmax(0,1.5fr)_100px_120px_120px_150px_110px] items-center gap-5 border-b border-black/10 bg-black/[.03] px-5 py-4 text-xs font-extrabold uppercase tracking-wide text-black/45 lg:grid"><span>Quiz</span><span>Status</span><span>Content</span><span>Activity</span><span>Audience</span><span className="justify-self-end">Actions</span></div>{quizzes.map(quiz=><article key={quiz.id} className="grid min-h-20 gap-4 border-b border-black/10 px-5 py-5 last:border-0 lg:grid-cols-[minmax(0,1.5fr)_100px_120px_120px_150px_110px] lg:items-center lg:gap-5"><div className="min-w-0"><Link href={`/m/${brand!.slug}/app/quizzes/${quiz.id}/preview`} className="font-extrabold uppercase hover:underline">{quiz.title}</Link><p className="mt-1 truncate text-sm text-black/45">{quiz.description||"No description added."} · {quiz.passing_score}% to pass</p><ContentGovernanceDetails record={governance.get(quiz.id)}/></div><div><span className={`inline-flex px-2 py-1 text-[10px] font-extrabold uppercase ${governanceStatusClass(quiz.status)}`}>{governanceStatusLabel(quiz.status)}</span><ContentScheduleDetails schedule={schedules.get(quiz.id)}/></div><div><span className="mr-2 text-[10px] font-extrabold uppercase text-black/35 lg:hidden">Content</span><b>{quiz.quiz_questions?.[0]?.count??0}</b><span className="ml-1 text-xs text-black/45">questions</span></div><div><span className="mr-2 text-[10px] font-extrabold uppercase text-black/35 lg:hidden">Activity</span><b>{attemptsByQuiz.get(quiz.id)??0}</b><span className="ml-1 text-xs text-black/45">attempts</span></div><div>{internalQuizIds.has(quiz.id)?<span className="inline-flex bg-blue-50 px-2 py-1 text-[10px] font-extrabold uppercase text-blue-800">Manufacturer team</span>:<span className="text-xs text-black/35">Unassigned</span>}</div><QuizCardActions quizId={quiz.id} status={quiz.status} hasAttempts={Boolean(attemptsByQuiz.get(quiz.id))} brandSlug={brand!.slug} assignedInternally={internalQuizIds.has(quiz.id)}/></article>)}</section>
      )}
    </main>
  </div>;
}
