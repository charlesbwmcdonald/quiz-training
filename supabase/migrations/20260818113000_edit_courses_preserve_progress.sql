create or replace function public.get_manufacturer_course_for_edit(target_course_id uuid)
returns jsonb
language sql
stable
security definer
...
as $function$
  select jsonb_build_object(
    ...
    'status', c.status,
    'has_activity', (
      exists(select 1 from public.company_course_assignments a where a.course_id = c.id)
      or exists(select 1 from public.manufacturer_course_assignments a where a.course_id = c.id)
      or exists(select 1 from public.course_block_progress p where p.course_id = c.id)
      or exists(select 1 from public.manufacturer_course_progress p where p.course_id = c.id)
    ),
    'blocks', coalesce(jsonb_agg(jsonb_build_object(
      ...
      'quiz_id', b.quiz_id,
      'required', b.required
    ) order by m.position, b.position), '[]'::jsonb)
  )
  from public.courses c
  join public.course_modules m on m.course_id = c.id
  join public.course_blocks b on b.module_id = m.id
...
begin
  ...
  -- Avoid the unique module/position constraint while retained blocks are reordered.
  ...
  -- Progress for retained blocks survives. Removing a block removes only that block's progress.
  delete from public.course_blocks b
  where b.module_id = target_module_id and not (b.id = any(retained_ids));
end;
$function$;

revoke all on function public.get_manufacturer_course_for_edit(uuid) from public;
grant execute on function public.get_manufacturer_course_for_edit(uuid) to authenticated;
